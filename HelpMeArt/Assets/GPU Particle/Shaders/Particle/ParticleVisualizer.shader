...
Shader "Unlit/ParticleVisualizer"
{
	...
	CGINCLUDE
		...
		v2f_shadow vertShadow(appdata v)
		{
			...
			v.vertex.xyz = mul(unity_WorldToObject, half4(pos.xyz,1)).xyz; // this is local space
			
			float4 wPos = mul(unity_ObjectToWorld, v.vertex);
			float3 wLitDir = UnityWorldSpaceLightDir( wPos.xyz );
			float3 lLitDir = mul(unity_WorldToObject, float4(wLitDir,0)).xyz;
			float3 lCamDir = mul(unity_WorldToObject, float4(_WorldSpaceCameraPos,1)).xyz;
